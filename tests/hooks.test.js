...
test('AE5: PLUGIN_DATA unset writes raw context with all three sections', () => {
  const r = run({});
...
test('AE5: PLUGIN_DATA set writes Codex hookSpecificOutput JSON', () => {
  const r = run({ PLUGIN_DATA: mkdtempSync(join(tmpdir(), 'od-')) });
  assert.equal(r.status, 0);
